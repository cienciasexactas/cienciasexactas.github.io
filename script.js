...
// ============================================================================
// 7. ENVIO DE FICHEIROS PARA A VERCEL (NOME EXATO DO FICHEIRO)
// ============================================================================
...
// ----------------------------------------------------------------------------
// 7.1. ALTERNAR CAMPOS ENTRE FICHEIRO E LINK
// ----------------------------------------------------------------------------
function alternarTipoEnvio() {
  const tipo = document.getElementById('select-tipo-envio')?.value;
  const containerFicheiro = document.getElementById('container-ficheiro');
  ...
  if (tipo === 'link') {
    if (containerFicheiro) containerFicheiro.style.display = 'none';
    if (containerReferencia) containerReferencia.style.display = 'block';
    if (containerUrl) containerUrl.style.display = 'block';
    if (btnEnviar) btnEnviar.textContent = 'Enviar Link';
  } else {
    if (containerFicheiro) containerFicheiro.style.display = 'block';
    if (containerReferencia) containerReferencia.style.display = 'none';
    if (containerUrl) containerUrl.style.display = 'none';
    if (btnEnviar) btnEnviar.textContent = 'Enviar Ficheiro';
  }
}

// ----------------------------------------------------------------------------
// 7.2. FUNÇÃO PRINCIPAL DE ENVIO (NOME EXATO DO FICHEIRO ORIGINAL)
// ----------------------------------------------------------------------------
async function enviarFicheiro(event) {
  ...
  const tipoEnvio = selectTipoEnvio ? selectTipoEnvio.value : 'ficheiro';

  // Validações
  if (!seccao) return alert('Por favor, selecione a Secção / Categoria.');
  if (!curso) return alert('Por favor, selecione o Curso.');
  if (!ano) return alert('Por favor, selecione o Ano Lectivo.');
  ...
  if (typeof PERIODOS_SUBMISSAO !== 'undefined' && PERIODOS_SUBMISSAO[seccao]) {
    ...
    if (agora < prazo.inicio || agora > prazo.fim) {
      return alert('O carregamento para esta secção encontra-se encerrado fora do período permitido.');
    }
  }
  ...
  if (tipoEnvio === 'link') {
    ...
    if (!url) {
      isUploading = false;
      return alert('Por favor, insira o Link / URL.');
    }

    const conteudoTexto = `Referência: ${referencia || 'N/A'}\nURL: ${url}`;
    const base64Content = btoa(unescape(encodeURIComponent(conteudoTexto)));

    const nomeLinkExato = `link_${(referencia || 'artigo').substring(0, 30).replace(/[^a-zA-Z0-9]/g, '_')}.txt`;

    const payload = {
      tipo: 'link',
      url: url,
      referencia: referencia,
      nomeArquivo: nomeLinkExato,
      content: base64Content,
      conteudoBase64: base64Content,
      curso: caminhoPastaDestino
    };

    await enviarPayloadParaVercel(payload, btnEnviar);
    return;
  }

  // SUBMISSÃO DE FICHEIRO FÍSICO (NOME EXATO)
  if (!inputFicheiro || !inputFicheiro.files[0]) {
    isUploading = false;
    return alert('Por favor, selecione um ficheiro primeiro.');
  }

  const ficheiro = inputFicheiro.files[0];
  const nomeExatoFicheiro = ficheiro.name;
...
// ----------------------------------------------------------------------------
// 7.3. AUXILIAR DE FETCH PARA A VERCEL
// ----------------------------------------------------------------------------
async function enviarPayloadParaVercel(payload, btnEnviar) {
  if (btnEnviar) {
    btnEnviar.disabled = true;
    btnEnviar.textContent = 'A processar...';
  }
  ...
  try {
    ...
    if (resposta.ok) {
      ...
      if (selectTipoEnvio) selectTipoEnvio.value = 'ficheiro';
      if (selectSeccao) selectSeccao.value = '';
      ...
      if (typeof carregarFicheirosExistentes === 'function') {
        await carregarFicheirosExistentes();
      }
    } else {
      alert('Erro no envio: ' + (dados.error || JSON.stringify(dados)));
    }
  } catch (erro) {
    ...
    if (btnEnviar) {
      ...
      const selectTipoEnvio = document.getElementById('select-tipo-envio');
      btnEnviar.textContent = selectTipoEnvio?.value === 'link' ? 'Enviar Link' : 'Enviar Ficheiro';
    }
  }
}
...
function inicializarSelectsFormulario() {
  // Preenchimento automático do select de tipo de envio via script
  const selectTipoEnvio = document.getElementById('select-tipo-envio');
  if (selectTipoEnvio && selectTipoEnvio.children.length === 0) {
    selectTipoEnvio.innerHTML = `
      <option value="ficheiro" selected>Ficheiro (Upload)</option>
      <option value="link">Link / URL (Site)</option>
    `;
    selectTipoEnvio.addEventListener('change', alternarTipoEnvio);
  }

  const seccoes = [
    { id: 'colaborativos', nome: 'Trabalhos Individuais' },
    { id: 'grupo', nome: 'Trabalhos de Grupo (Pesquisa)' },
  ...
  const cursos = [
    ...
    { id: 'economia', nome: 'Licenciatura em Gestão de Empresa' },
    ...
    try {
      ...
      for (const blocoAno of estruturaComFicheiros) {
        ...
        tabela.innerHTML = `
          <thead>
            <tr style="background-color: #f8f9fa; border-bottom: 2px solid #dee2e6; text-align: left;">
              <th style="padding: 10px; width: 60%; white-space: nowrap;">Nome / Referência</th>
              <th style="padding: 10px; width: 20%; white-space: nowrap;">Data de Envio</th>
              <th style="padding: 10px; width: 20%; white-space: nowrap;">Tamanho / Tipo</th>
            </tr>
          </thead>
          <tbody></tbody>
        `;
        ...
        for (const item of blocoAno.ficheiros) {
          ...
          const isLink = item.tipo === 'link' || item.isLink || item.nome?.startsWith('link_') || item.nome?.endsWith('.txt');

          let textoReferencia = item.referencia || item.nome;
          let urlDestino = item.url;

          if (isLink && item.url) {
            try {
              const resFicheiro = await fetch(item.url);
              if (resFicheiro.ok) {
                const conteudoTexto = await resFicheiro.text();
                const matchRef = conteudoTexto.match(/Referência:\s*(.*)/i);
                const matchUrl = conteudoTexto.match(/URL:\s*(.*)/i);

                if (matchRef && matchRef[1].trim()) {
                  textoReferencia = matchRef[1].trim();
                }
                if (matchUrl && matchUrl[1].trim()) {
                  urlDestino = matchUrl[1].trim();
                }
              }
            } catch (errLink) {
              console.warn('Erro ao ler o conteúdo do link:', errLink);
            }
          }
          ...
          // ------------------------------------------------------------------
          // VERIFICAÇÃO AUTOMÁTICA DE 1 MÊS APÓS O ENVIO
          // ------------------------------------------------------------------
          const umMesAposEnvio = new Date(dataEnvioFicheiro);
          umMesAposEnvio.setMonth(umMesAposEnvio.getMonth() + 1);

          let linkBloqueado = false;

          // Se ainda estiver dentro do primeiro mês de vida do documento, aplica as regras globais
          if (agora < umMesAposEnvio) {
            ...
            // Passou 1 mês desde a submissão: desbloqueia automaticamente
            linkBloqueado = false;
          }

          const comandoPopup = `window.open(this.href, 'janelaPopup', 'width=900,height=700,scrollbars=yes,resizable=yes'); return false;`;

          if (linkBloqueado) {
            conteudoColunaNome = `
              <span style="color: #6c757d; cursor: not-allowed;" title="O link ficará disponível após o encerramento do prazo ou após 1 mês da submissão.">
                ${isLink ? textoReferencia : item.nome} 🔒 <em style="font-size: 0.85em; color: #d93025; font-style: normal;">(Pendente)</em>
              </span>`;
          } else if (isLink) {
            conteudoColunaNome = `
              <span>${textoReferencia}</span> 
              <a href="${urlDestino}" onclick="${comandoPopup}" style="color: #0066cc; font-weight: bold; text-decoration: underline; margin-left: 4px;">
                (Aceder)
              </a>`;
          } else {
            conteudoColunaNome = `
              <a href="${item.url}" onclick="${comandoPopup}" style="color: #0066cc; text-decoration: underline;">
                ${item.nome}
              </a>`;
          }

          const tamanhoTexto = isLink ? 'Link Externo' : formatarTamanho(item.tamanho);
          ...
          tr.innerHTML = `
            ...
            <td style="padding: 10px; color: #555; white-space: nowrap;">${tamanhoTexto}</td>
          `;
